...
public class DefaultLoanApprovalServiceTest {
    ...
    public void workItemSetup() {
        RuleBaseConfiguration ruleBaseConfiguration = ((AbstractRuleBase) knowledgeBase).getConfiguration();
    ...
    public void amountToBorrowLow() {
        ...
        assertFalse(trackingProcessEventListener.isNodeTriggered(PROCESS_LOAN_APPROVAL,
    ...
    public void amountToBorrowHighRatingCalculation() {
        setUpHighAmount();
        startProcess();
        assertTrue(trackingProcessEventListener.isNodeTriggered(PROCESS_LOAN_APPROVAL,
                NODE_SUBFLOW_RATING_CALCULATION));
        assertTrue(trackingProcessEventListener.isNodeTriggered(
    ...
    public void ratingCalculation() {
        ...
        assertNotNull(session.getObjects(
                new ClassObjectFilter(Rating.class)).iterator().next());
    }
    ...
    public void processLoan() throws Exception {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("droolsBook.droolsFlow");
        ...
        while ((server.getIoAcceptor() == null || !server.getIoAcceptor().isActive()) && timeoutCounter++ < 1000) {
            Thread.sleep(10);
        }
        ...
        do {
            client.getTasksAssignedAsPotentialOwner("123", "en-UK", summaryHandler);
        ...
        operationHandler = new BlockingTaskOperationResponseHandler();
        client.complete(task.getId(), "123", null, operationHandler);
        operationHandler.waitTillDone(5000);

        //wait until the process resumes or timeout expires
        timeoutCounter = 0;
        while (!trackingProcessEventListener.isNodeTriggered(PROCESS_LOAN_APPROVAL, NODE_JOIN_PROCESS_LOAN)
                && timeoutCounter++ < 1000) {
            Thread.sleep(10);
        }
